...
class Main { 
	...
	Main(String s1, String s2, String s3, String s4) { 
	 	mnemonic = s1;  	 	
	 	bin_opcode = s2; 
...
class SymMain { 
	...
	SymMain(String s1, int i1, int i2, String s2) { 
	 	symbol = s1; 
	 	value = i1; 
	 	length = i2; 
	 	ra = s2; 
...
class LitMain { 
	...
	LitMain(String s1, int i1, int i2, String s2) { 
	 	literal = s1; 
	 	value = i1; 
	 	length = i2; 
	 	ra = s2; 
...
class PassOne{ 
 static int lc; 
 static List<Main> mot; 
 static List<String> pot; 
 static List<SymMain> symtable; 
 static List<LitMain> littable; 
 ...
 static boolean searchPot1(String[] s) { int i = 0; 
	 	int l = 0; 
	 	int potval = 0; 
	 	...
	 	switch(potval) {  	 	 	
	 	    case 1: 
	 	 	 	...
	 	 	 	if(i == 1) { 
	 	 	 	 	symtable.add(new SymMain(s[0], lc, 4, "R")); 
	 	 	 	} 
	 	 	 	if(index != 0) {  	 	 	 	 	// Ends with F 
	 	 	...
	 	 	case 2: 
	 	 	 	// EQU statement 
	 	        if(!s[2].equals("*")) { 
	 	 	 	    symtable.add(new SymMain(s[0], 
                 Integer.parseInt(s[2]), 1, "A")); 
	 	 	 	} 
	 	 	 	else { 
	 	 	 	 	symtable.add(new SymMain(s[0], lc, 1, "R")); 
	 	 	 	} 
	 	 	 	return true; 
	 	 	 
	 	 	case 3: 
	 	 	 	// START statement  	 	 	 	
	 	 	 	symtable.add(new SymMain(s[0],Integer.parseInt(s[2]), 1, "R"));
	 	 	 	return true; 
	...
	static void searchMot1(String[] s) {  	 	
	    ...
	    if(s.length == 3) { 
	 	...
	 	for(int j=i+1 ; j < s.length ; j++) {  	 	 	
	 	    if(s[j].startsWith("=")) { 
             littable.add(new LitMain(s[j].substring(1, s[j].length()), -1, 4, "R")); 
	 	 	} 
	 	} 
 ...
 static void ltorg(boolean isEnd) { 
	 	Iterator<LitMain> itr = littable.iterator(); 
	    LitMain lt = new LitMain();  	
 ...
 static void searchMot2(String[] s) {  	
	...
	static void initializeTables() throws Exception { 
     ...
     while((s = br.readLine()) != null) { 
         StringTokenizer st = new StringTokenizer(s, " ", false); 
	 	 	mot.add(new Main(st.nextToken(), st.nextToken(), st.nextToken(), st.nextToken())); 
	 	} 
